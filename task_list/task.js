...
(function () {
    ...
    listAllTasks(stack) {
      ...
      taskList.forEach((item) => {
        ...
        div.innerHTML = `
         ...
         <h4 class='singleTask_taskNumber'> Answer:<span class="${answerStyles} hide">${item.solutionValue}</span></h4>
  ...
  `;
    ...
    deleteSingleTask(singleTask, taskID) {
      this.removeFromDOM(singleTask);
    }
    removeFromDOM(task){
  ...
  taskArea.addEventListener("click", (event) => {
    if (event.target.classList.contains("btnShow")) {
      ...
      ui.deleteSingleTask(singleTask, id);
    }
  });
  /* ------------------------------ EVENTI ------------------------------ */
})();
